...
test_1password_service_account_token_skips_signin() {
  local tmp_dir
  local bin_dir
  local op_log
  local secrets_file
...
case "$1" in
  ...
  environment)
    ...
    if [ "${OP_SERVICE_ACCOUNT_TOKEN:-}" != "service-token" ]; then
      exit 4
    fi

    printf 'TEST_SECRET=service token value\n'
    ;;
  *)
    exit 5
    ;;
...
EOF
  ...
  (
    export HOME="$tmp_dir/home"
    export OP_SERVICE_ACCOUNT_TOKEN="service-token"
    export OP_TEST_LOG="$op_log"
    export PATH="$bin_dir:$PATH"
  ...
  if [ -f "$op_log" ]; then
    printf 'expected op signin not to be invoked with service account token\n' >&2
    return 1
  fi

  if ! grep -q '^TEST_SECRET="service token value"$' "$secrets_file"; then
    printf 'expected service-account secrets file to include quoted output\n' >&2
    return 1
  fi
